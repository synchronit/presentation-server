...
wholeApp.controller('importerController', ['$scope', '$http', 'broadcastService', function ($scope, $http, $broadcastService)
    {
        ...
        $('select#mapping_names').on('change', function () {
            ...
            if (selectedValue == 'Select mapping') {
                $('table#table_mapping select').each(function (index, elem) {
                    $(this).val('ignore')
                });
                isUsingExistingMap = false;
            } else {
        ...
        var updateMappingSelect = function (formName) {
            ...
            var filtered = $.savedMappings.filter(function (item) {
        ...
        var loadExistingMapping = function (selectedMapping) {
            var map = $.savedMappings.filter(function (item) {
                return item.sourceName == selectedMapping ? true : false
            })[0];
        ...
        var registerInputs = function () {
            $('input[type="file"]', '#table_mapping').bind('change', function (event) {
        ...
        var hasColumnMapped = function () {
            var notMappedColumnCount = $('select.form_columns').children('option:selected').filter(function (i, e) {
                if ($(this).html() === "ignore") {
                    return true
                } else {
                    return false
            ...
            if (columnCount() == notMappedColumnCount) {
                return false
            } else {
                return true
            }
        ...
        var checkExistingMap = function (mapName) {
            return $.savedMappings.filter(function (item) {
                return item.sourceName == mapName ? true : false
            }).length == 0 ? false : true;
        };
